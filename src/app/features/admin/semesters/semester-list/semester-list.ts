...
export class SemesterListComponent implements OnInit {
  public service = inject(SemesterService);
  ...
  readonly academicYears = this.academicYearService.items;
  ...
  readonly editTarget = signal<Semester | null>(null);
  editForm!: FormGroup;
  ...
  ngOnInit(): void {
    this.academicYearService.fetchAll().subscribe();
  ...
  onSubmit(): void {
    ...
    this.service.create(this.addForm.value).subscribe({
      next: () => {
        ...
        this.closeAddModal();
        this.service.fetchAll().subscribe();
        this.toast.success('Semester added successfully!');
      },
      error: () => {
        this.isSubmitting.set(false);
  ...
  openEditModal(item: Semester): void {
  ...
  onSubmitEdit(): void {
    ...
    this.service.update(target.semester_id, this.editForm.value).subscribe({
      next: () => {
  ...
  confirmDelete(): void {
    ...
    this.service.delete(target.id).subscribe({
      ...
      error: () => {
        this.isDeleting.set(false);
        this.toast.error('Cannot delete semester. Please try again.');
      },
    });
  }
}
