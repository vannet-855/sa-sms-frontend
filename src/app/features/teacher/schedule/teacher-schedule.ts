    ...
    <div class="flex min-h-screen bg-slate-950">
      ...
      <main class="flex-1 overflow-y-auto p-6">
        ...
        <div class="mb-1 text-xs text-slate-600">
          <span>Teacher</span> / <span class="text-emerald-400">My Schedule</span>
        </div>
        ...
        @if (!loading() && !error()) {
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-3 xl:grid-cols-6">
            @for (day of dayNames; track day.key; let first = $first) {
                ...
                <div class="space-y-2 p-3">
                  @if (scheduleByDay()[day.key]?.length) {
                    ...
                    <div class="py-6 text-center text-xs text-slate-600">
                      <i class="fa-regular fa-calendar mb-1 block"></i>
                      No classes
                    </div>
                  }
                </div>
                  ...
                  @for (entry of schedules(); track entry.schedule_id) {
                    ...
                    <tr>
                      <td colspan="6" class="px-4 py-12 text-center text-slate-600">
                        <i class="fa-regular fa-calendar mb-2 block text-lg"></i>
...
export class TeacherScheduleComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    for (const day of this.dayNames) {
      day.isToday = day.key === this.todayKey;
    }
    ...
    this.http
      .get<ScheduleEntry[]>(`${environment.apiUrl}/schedules/teacher`)
      .pipe(
        catchError((err) => {
